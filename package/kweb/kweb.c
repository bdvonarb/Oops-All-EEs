/*                    Minimal Kiosk Browser 
   ...
   Version 1.7.0 using gtk+2 and webkit-1.0
*/
...
static gchar* fs_u = "u";
static gchar* fs_f = "f";
static gchar* max_m = "m";
static gchar* max_u = "u";
static gchar* default_ua = "Mozilla/5.0 (X11; Linux armv6l) AppleWebKit/538.15 (KHTML, like Gecko) Version/8.0 Safari/538.15 kweb/1.7.0";
...
static void show_title(WebKitWebView  *webView,  WebKitWebFrame *frame, GtkWidget* window) {
	...
	if (title != NULL) {
		gchar * ntitle = g_strjoin(NULL,title," (kweb)",NULL);
...
static void new_browser(const gchar* uri) {
	...
	if (r == 0) {
		if (command_flags == NULL) {
			execl("/usr/bin/kweb", "kweb", uri, NULL); }
		else {
                	execl("/usr/bin/kweb", "kweb", command_flags, uri, NULL); }
        }
}
...
static void open_file (GtkWidget *widget, gpointer user_data) {
	...
	GtkWidget* entry = g_object_get_data(G_OBJECT(file_selector), "uriEntry");
	gchar* fpuri;
	struct stat sb;
...
static void select_file(GtkWidget* item, GtkWidget* entry )
{
	...
	file_selector = gtk_file_selection_new ("Select a file to open");
	g_object_set_data( G_OBJECT(file_selector), "uriEntry", entry);
	if (strlen(last_filepath) != 0) {
		gtk_file_selection_set_filename(GTK_FILE_SELECTION (file_selector), last_filepath);
	}
...
/*			downloadPage(window, webView); */
			...
			stopLoading(window, webView);
                ...
                case 'm':
/*			loadUri(webView,"file:///usr/local/share/kweb/kweb_about_m.html"); */
	        	max = g_object_get_data(G_OBJECT(window), "maximized");
			if (strcmp(max, "u") == 0) {
				g_object_set_data( G_OBJECT(window), "maximized",max_m );
				gtk_window_maximize(GTK_WINDOW(window)); }
			else {
				g_object_set_data( G_OBJECT(window), "maximized",max_u);
                                     ...
                                     gchar* arg)
{
	...
	gtk_widget_set_tooltip_text(GTK_WIDGET(uriEntry),"Enter URL (i)\n?web search\n=page search\n#command line\n:command\n!language\n@encoding\n$user-agent\n&user style URI\n)URL new window\n))URL new browser");
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"document-open");
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Open file (o)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"go-previous");
        gtk_toolbar_insert(GTK_TOOLBAR( toolbar ), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Back to last page (b)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"process-stop");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Stop loading (s)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"view-refresh");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Reload page (r)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"go-home");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Open your homepage (h)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"media-playback-start");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Play video from website (p)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"zoom-in");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Zoom in by 10% (+)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"zoom-out");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
	gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Zoom out by 10% (-)", NULL);
	...
	gtk_tool_button_set_icon_name (GTK_TOOL_BUTTON( item ),"zoom-original");
        gtk_toolbar_insert(GTK_TOOLBAR(toolbar), item, -1);
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Reset zoom to 100% (z)", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Full Zoom");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Global, full zoom or text zoom only (g,t)", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Javascript");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Javascript or No Javascript (j,n)", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Cookies");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Enable/disable cookies (e,d)", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"DL-Manager");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," uGet or wget for downloads (u,w)", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Omxplayer");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Use omXplayer for media (x,y)", NULL);
	...
	gtk_tool_button_set_label (GTK_TOOL_BUTTON( item ),"Commands");
        ...
        gtk_tooltips_set_tip(tooltips, GTK_WIDGET(item)," Activate/veto command execution (a,v)", NULL);
